...
# Histogramas → AD e Gender
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Subplot 1: Distribuição de Alzheimer (AD)
sns.countplot(data=df,x='AD', hue='AD', ax=axes[0], palette='Set2',legend=False)
...
# Subplot 2: Distribuição de Gênero (GENDER)
...
# Histograma das idade+diagnóstico
plt.figure(figsize=(10, 6))

# O parâmetro hue separa por AD, e multiple="stack" empilha as barras
sns.histplot(data=df, x='Age', hue='AD', bins=15, multiple='stack', palette='Dark2', edgecolor='white')

plt.title('Distribuição de Idade Estratificada por Alzheimer (AD)')
plt.xlabel('Idade (Anos)')
plt.ylabel('Número de Casos')
# Ajustando a legenda para ficar clara
...
rois = df.columns[2:-3]  
# print("ROIs disponíveis: ", rois.tolist())

# =================================================
# CORRELAÇÃO entre ROIs (é esperado que haja muita correlação entre elas,
# # o que pode levar a problemas de multicolinearidade)

plt.figure(figsize=(12, 10))

# cálculo da correlação de Pearson entre as ROIs
matriz_corr = df[rois].corr(method='pearson')
# Criando uma máscara para esconder o triângulo superior 
mask = np.triu(np.ones_like(matriz_corr, dtype=bool))
 # np.triu → preserva os valores do triangulo superior (incluindo a diagonal) como 1
 # e zera os do inferior, criando uma matriz booleana para a máscara

# Plotando o Heatmap
sns.heatmap(
    matriz_corr, 
    mask=mask, 
    cmap='coolwarm',       # Paleta divergente ideal para correlação (-1 azul, 0 branco, 1 vermelho)
    vmin=-1, vmax=1,       # Força os limites da escala de cores
    center=0,
    xticklabels=False,     # Desativado para não poluir, já que são 91 colunas
    yticklabels=False,     # Desativado para não poluir
    cbar_kws={"shrink": 0.8, "label": "Coeficiente de Correlação de Pearson"}
)
...
# pegando os valores absolutos da matriz de correlação das ROIs
matriz_corr_abs = matriz_corr.abs()
# criando uma máscara para pegar apenas a parte inferior da matriz e sem a diagonal
#(ROI A e ROI B = ROI B e ROI A // correlação de uma ROI consigo mesma = 1)
mask_abs = np.triu(np.ones_like(matriz_corr_abs, dtype=bool))
# aplicando a máscara para obter apenas as correlações únicas
correlacoes_unicas = matriz_corr_abs.where(~mask_abs) #'~' inverte os booleanos da máscara 
# encontrando os pares de ROIs com correlação acima do limite
...
print(f"Pares de ROIs com correlação absoluta maior que {limite_correlacao}:")
print(pares_correlacionados_filtrados.sort_values(by='Correlacao', ascending=False).head(30))  # Mostra os x pares mais correlacionados


# =====================================================================
# Teste estatístico para testar se a diferença das médicas entre o grupo controle e AD é 
# estatisticamente significativa (p_value>0.05)
# =====================================================================
resultados = []
...
print("As 10 ROIs com maior diferença estatística entre os grupos:")
print(df_testes.head(10))
...
# Loop para gerar um gráfico de 2 subplots (Esquerdo vs Direito) para cada região
for nome_regiao, (roi_esq, roi_dir) in regioes_analise.items():
    
    # Calcula a porcentagem de overlap para cada lado
    overlap_esq = calcular_overlap(df, roi_esq)
    overlap_dir = calcular_overlap(df, roi_dir)
    
    # Figura com 1 linha e 2 colunas
    fig, axes = plt.subplots(1, 2, figsize=(15, 7))
    fig.suptitle(f'Análise de Densidade Volumétrica: {nome_regiao}', fontsize=16, fontweight='bold', y=0.98)
    
    # Subplot 1: Hemisfério Esquerdo
    sns.kdeplot(
        data=df, x=roi_esq, hue='AD', fill=True, common_norm=False, 
        palette='Set1', alpha=0.4, ax=axes[0])
    # Adicionando a métrica no título do subplot
    axes[0].set_title(f'Hemisfério Esquerdo ({roi_esq})\nInterseção das Curvas: {overlap_esq:.2f}%', fontsize=12)
    axes[0].set_xlabel('Volume Normalizado')
    axes[0].set_ylabel('Densidade')
    
    # Subplot 2: Hemisfério Direito
    sns.kdeplot(
        data=df, x=roi_dir, hue='AD', fill=True, common_norm=False, 
        palette='Set1', alpha=0.4, ax=axes[1])
    # Adicionando a métrica no título do subplot
    axes[1].set_title(f'Hemisfério Direito ({roi_dir})\nInterseção das Curvas: {overlap_dir:.2f}%', fontsize=12)
    axes[1].set_xlabel('Volume Normalizado')
    axes[1].set_ylabel('Densidade')
    
    # Ajustando legendas internas
    for ax in axes:
        legend = ax.get_legend()
        if legend:
            legend.set_title("Grupo (AD)")
            new_labels = ['Alzheimer (1)' if t.get_text() == '1' else 'Controle (0)' for t in legend.get_texts()]
...
# Gráfico único ara comparação da Amígdala Esquerda dos dois grupos: ----------------------------------------------------
overlap_amigdala = calcular_overlap(df, 'Left-Amygdala')

plt.figure(figsize=(9, 6))
sns.kdeplot(
    data=df, x='Left-Amygdala', hue='AD', fill=True, common_norm=False, 
    palette='Set1', alpha=0.4)
# títulos, labels
plt.title(f'Distribuição Volumétrica da Amígdala Esquerda\nInterseção das Curvas: {overlap_amigdala:.2f}%', fontsize=14, fontweight='bold')
plt.xlabel('Volume Normalizado (Left-Amygdala)', fontsize=12)
plt.ylabel('Densidade', fontsize=12)
#
figura_atual = plt.gca()
legend = figura_atual.get_legend()
if legend:
    legend.set_title("Grupo (AD)")
    new_labels = ['Alzheimer (1)' if t.get_text() == '1' else 'Controle (0)' for t in legend.get_texts()]
    for text, new_label in zip(legend.get_texts(), new_labels):
...
#-----------------------------------------------------------------------------------------------
# Análise de densidade volumétrica dos ventrículos pareados =======================================
...
for nome_regiao, (roi_esq, roi_dir) in ventriculos_pareados.items():
    
    # Calcula o overlap para cada hemisfério
    overlap_esq = calcular_overlap(df, roi_esq)
    overlap_dir = calcular_overlap(df, roi_dir)
...
# Calcula o overlap para o 3º e 4º ventrículo
overlap_3rd = calcular_overlap(df, '3rd-Ventricle')
overlap_4th = calcular_overlap(df, '4th-Ventricle')

# Cria a figura com 1 linha e 2 colunas
fig, axes = plt.subplots(1, 2, figsize=(15, 7))
fig.suptitle('Análise de Densidade Volumétrica: Ventrículos Centrais (Linha Média)', fontsize=16, fontweight='bold', y=0.98)

# Subplot 1: Terceiro Ventrículo
sns.kdeplot(
    data=df, x='3rd-Ventricle', hue='AD', fill=True, common_norm=False, 
    palette='Set1', alpha=0.4, ax=axes[0])
axes[0].set_title(f'Terceiro Ventrículo (3rd-Ventricle)\nInterseção das Curvas: {overlap_3rd:.2f}%', fontsize=12)
axes[0].set_xlabel('Volume Normalizado')
axes[0].set_ylabel('Densidade')

# Subplot 2: Quarto Ventrículo
sns.kdeplot(
    data=df, x='4th-Ventricle', hue='AD', fill=True, common_norm=False, 
    palette='Set1', alpha=0.4, ax=axes[1])
axes[1].set_title(f'Quarto Ventrículo (4th-Ventricle)\nInterseção das Curvas: {overlap_4th:.2f}%', fontsize=12)
axes[1].set_xlabel('Volume Normalizado')
axes[1].set_ylabel('Densidade')
...
# Criando um painel de 2x2 subplots
...
# Dicionário de cores para manter consistência (Azul para Controle, Vermelho para Alzheimer)
cores_grupo = {0: '#1f77b4', 1: '#d62728'}
labels_grupo = {0: 'Controle (0)', 1: 'Alzheimer (1)'}
...
for i, roi in enumerate(rois_dispersao):
    ...
    for grupo, dados_grupo in df.groupby('AD'):
        ...
        sns.scatterplot(
            data=dados_grupo, x='Age', y=roi, 
            color=cores_grupo[grupo], alpha=0.5, ax=axes[i],
            label=labels_grupo[grupo] if i == 0 else "" # Adiciona legenda apenas no primeiro plot para não duplicar
        )
        # Reta de tendência linear (ci=None remove a sombra do intervalo de confiança para limpar o gráfico)
        sns.regplot(
            data=dados_grupo, x='Age', y=roi, scatter=False, 
            color=cores_grupo[grupo], ax=axes[i], ci=None
        )
    
    # Customização de cada subplot
    axes[i].set_title(f'Dinâmica do Envelhecimento: {roi}', fontsize=12, fontweight='bold')
    axes[i].set_xlabel('Idade (Anos)', fontsize=10)
    axes[i].set_ylabel('Volume Normalizado', fontsize=10)

# Ativa a legenda global configurada no primeiro gráfico
...
for i, (nome_regiao, (roi_esq, roi_dir)) in enumerate(regioes_pareadas.items()):
    ...
    # Calculando o Índice de Assimetria formal (AI)
    coluna_ai = f'AI_{nome_regiao}'
    df[coluna_ai] = ((df[roi_esq] - df[roi_dir]) / (df[roi_esq] + df[roi_dir])) * 100
    
    # Plotando o Boxplot estruturado sem warnings de paleta
    sns.boxplot(data=df, x='AD', y=coluna_ai, hue='AD', palette='Set2', ax=axes[i], legend=False)
    
    # Opcional: adiciona mini-pontos pretos transparentes para mostrar a distribuição real dos pacientes
    sns.stripplot(data=df, x='AD', y=coluna_ai, color='black', alpha=0.2, size=3, ax=axes[i])
    ...
    # Adiciona uma linha pontilhada no zero (Simetria Perfeita) para referência visual
